...
Version 1.3.1 is available from the same public repository URL used by earlier releases. Existing VCC and ALCOM users do not need to add a new repository: refresh the existing `Avatar Recovery Unity` repository, then update `Avatar Recovery` to version 1.3.1.
...
The current package is `com.nickel-jp.avatar-recovery-1.3.1.zip`.
After downloading the ZIP, verify the published hash before importing it:

```powershell
# 1. Calculate the ZIP SHA-256 hash.
(Get-FileHash .\com.nickel-jp.avatar-recovery-1.3.1.zip -Algorithm SHA256).Hash

# Confirm that it is 44E6C298D61C4A62F2CC7A67B7C29D87B824ED1390557AF05C6D21835581F681
# and matches the packages/com.nickel-jp.avatar-recovery-1.3.1.zip entry in
# checksums/com.nickel-jp.avatar-recovery-1.3.1.sha256.txt.
```

To verify the signed DLL, extract the package and compare the signer thumbprint with the published certificate:

```powershell
Expand-Archive .\com.nickel-jp.avatar-recovery-1.3.1.zip -DestinationPath .\avatar-recovery-verify -Force
